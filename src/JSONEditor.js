...
var JSONEditor = function (container, schema, data, cbObj) {
  ...
  var ajv = new Ajv({allErrors: true/*, extendRefs: false*/})
  ...
  var iterateLevel = (obj, container, ctx, ctxPath, parentKey, levelNumber, parentCtx, itemArrayIndex, parentObj)  => {
    

    //if (obj.definitions && obj['$ref']) {
    //  obj = this.mjp(obj['$ref'], this.rootSchemaAlt);
    //}

    if (obj.items && obj.items["$ref"]) {
      var ref = this.mjp(obj.items["$ref"], this.rootSchemaAlt);
      Object.keys(ref).map((k) => obj.items[k] = _.clone(ref[k]));
      delete obj.items["$ref"];
    }
    ...
    if (obj.type === 'object') {
      ...
      if (!ctxContainerExists) {
        ctxContainer.className += 'box is-radiusless '
      ...
      Object.keys(obj.properties).map((key) => {
        ...
        // user can have options to chooche object props
        if (obj.options && obj.options.selectPropeties) {
          var objPropsContainerEl = this.rootContainer.querySelector('[path="'+ctxPath+'"]._json-editor-props')
          if (!objPropsContainerEl) {
            var objPropsContainerLabelEl = document.createElement('span')
            objPropsContainerLabelEl.innerHTML = 'Properties: '
            objPropsContainerLabelEl.className += 'has-text-grey-dark is-size-6 '
            ctxContainer.append(objPropsContainerLabelEl)

            objPropsContainerEl = document.createElement('span')
            objPropsContainerEl.className += '_json-editor-props'
            objPropsContainerEl.setAttribute('path', ctxPath)
            ctxContainer.append(objPropsContainerEl)


          }

          var inputPropEl = objPropsContainerEl.querySelector('[key="'+key+'"]')
          if (!inputPropEl) {

            inputPropEl = document.createElement('input')
            inputPropEl.setAttribute('key', key)
            inputPropEl.className += 'checkbox'
            inputPropEl.setAttribute('type', 'checkbox')
            inputPropEl.checked = ctx[key] !== undefined;
            
            var labelPropEl = document.createElement('label')
            //labelPropEl.append(inputPropEl)
            objPropsContainerEl.append(inputPropEl)
            labelPropEl.innerHTML += key
            
            objPropsContainerEl.append(labelPropEl)

            inputPropEl.addEventListener('change', (e) => {

              if (!e.target.checked) {
                delete ctx[key]
                var tgtCtx = this.rootContainer.querySelector('[path="'+ctxPath+'.'+key+'"]._json-editor')
                tgtCtx.remove()
              } else {
                ctx[key] = ''// TODO: set correct val for type
              }
              iterateLevel(this.rootSchema, this.rootContainer, this._data.root)
            })
          }
        ...
        if (ctx[key] === undefined) {
          ...
          if (parentObj && parentObj.type === 'object') {
            ctx = {}
    ...
    if (obj.type === 'string' || obj.type === 'integer' || obj.type === 'enum') {
      if (!ctxContainerExists) {
          ...
          if (obj.options && obj.options.format === 'code') {
            ...
            inputEl.onkeydown = function (e){
              if(e.key === 'Tab') {
                ...
                // unident tab 
                if (e.shiftKey) {
                  This commented code is to enable unident. 
                  If works but with some bugs affecting the last line of the editor
                  ...
                  if (e.target.selectionStart ===  e.target.selectionEnd) {
                    ...
                    for (var i = 0; i < lines.length; i++) {
                      ...
                      if (charCount > selection) {
                        ...
                        // remove if exists spaces on the left of cursor
                        ...
                        if (colPosition > 0 && contentOnLeftOfCursor.trim() === '') {
                          ...
                          e.target.value = lines.map((line, j) => (j === lines.length-1) ? line : line.slice(0, -1))
                            .join('\n')
                          ;
      ...
      if (inputEl.nodeName === 'SELECT') {
        ...
        if (['number', 'integer'].indexOf(obj.type) !== -1) {
          inputEl.value = ctx || null;
        } else {
          inputEl.value = (ctx === undefined) ? (obj.default || '') : ctx;
        }
      }
